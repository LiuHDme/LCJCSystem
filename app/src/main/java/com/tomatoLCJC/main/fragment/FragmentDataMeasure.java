        ...
        backBtn.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                if (!fromScanChart) {
                    ...
                    // 缩放
                    double scale = scanView.xDistance /
                            (mService.getMultipleSeriesRenderer().getXAxisMax() -
                                    mService.getMultipleSeriesRenderer().getXAxisMin());
                    scanView.xScale = (float) scale;
                    scanView.yScale = (float) scale;
                }
                // 同步折线图
                double xTrans = scanView.xTranslate / canvasWidth * scanView.xDistance;
                double xScaleDis = (scanView.xDistance / scanView.xScale - scanView.xDistance) / 2;
                FragmentDataMeasure.getInstance().mService.getMultipleSeriesRenderer()
                        .setXAxisMin(-xTrans - xScaleDis);
                FragmentDataMeasure.getInstance().mService.getMultipleSeriesRenderer()
                        .setXAxisMax(scanView.xDistance - xTrans + xScaleDis);
                // 重绘折线图
    ...
    public void initChartAndChannelButtonNum(int channelNum){
                ...
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        if (event.getPointerCount() == 1) {
                            if (!flag) {
                                if (event.getX() > xDown) {
                                    scanView.xTranslate += (event.getX() - xDown) / scanView.xScale;
                                    xDown = event.getX();
                                } else if (event.getX() < xDown) {
                                    scanView.xTranslate -= (xDown - event.getX()) / scanView.xScale;
                                    xDown = event.getX();
                                }
                                break;
                            }
                        } else if (event.getPointerCount() == 2) {
                            double x = mService.getMultipleSeriesRenderer().getXAxisMax() -
                                    mService.getMultipleSeriesRenderer().getXAxisMin();
                            scanView.xScale = (float) (scanView.xDistance / x);
                            scanView.yScale = (float) (scanView.xDistance / x);
                            break;
                        }
                    // 两根手指中的一根抬起时
                    case 262:
        ...
        primary_curve.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                scanView.xScale = 1;
                scanView.yScale = 1;
                scanView.xTranslate = 0;
                scanView.yTranslate = 0;
            }
        });
