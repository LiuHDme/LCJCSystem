...
public class MainActivity extends FragmentActivity implements OnClickListener {
    ...
    public final Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            if (msg.what==2){
                m_vFragmentDataMeasure.mService.refreshChart();
                m_vSecondFragmentDataMeasure.mService.refreshChart();
                m_vSecondFragmentDataMeasure.mService1.refreshChart();
            }
        }
    };
        ...
        public void run() {
            ...
            for (int i=0;i<ThreadParameter.getInstance().xList.size();i++){
                for(int j=0;j<SystemParameter.getInstance().nChannelNumber;j++){
                    tempValue=quadratic_1.argTOdep(ThreadParameter.getInstance().denoisingValue.get(j).get(i));
                    if (tempValue<=40){
                        ThreadParameter.getInstance().flawValue.get(j).add(tempValue);
                    } else{
                        ThreadParameter.getInstance().flawValue.get(j).add(quadratic_2.argTOdep(ThreadParameter.getInstance().denoisingValue.get(j).get(i)));
                    }
                }
            }
//            Log.d("yList.size()",String.valueOf(ThreadParameter.getInstance().yList.size()));
//            for(int j=0;j<SystemParameter.getInstance().nChannelNumber;j++){
//                Log.d("yList.get("+j+").size()",String.valueOf(ThreadParameter.getInstance().yList.get(j).size()));
//            }
            m_vFragmentDataMeasure.mService.drawMeasureChart(ThreadParameter.getInstance().xList,ThreadParameter.getInstance().denoisingValue );          //去噪折线图
            handler.post(drawScanChart);    // 绘制扫描图
            m_vSecondFragmentDataMeasure.mService.drawMeasureChart(ThreadParameter.getInstance().xList,ThreadParameter.getInstance().detectionValue);     //检测折线图
            m_vSecondFragmentDataMeasure.mService1.drawMeasureChart(ThreadParameter.getInstance().xList,ThreadParameter.getInstance().gradientValue);     //横向梯度折线图
    ...
    // 改变 UI，绘制扫描图
    Runnable drawScanChart = new Runnable() {
        @Override
        public void run() {
            m_vFragmentDataMeasure.scanView = new ScanningService(
                    m_vFragmentDataMeasure.getContext(),
                    ThreadParameter.getInstance().xList,
                    ThreadParameter.getInstance().yList,
                    ThreadParameter.getInstance().flawValue,
                    true);
            m_vFragmentDataMeasure.scanChart.removeAllViews();
            m_vFragmentDataMeasure.scanChart.addView(m_vFragmentDataMeasure.scanView);
            m_vFragmentDataMeasure.scanView.setClickable(true);
        }
    };

    // 改变 UI，清空扫描图
    Runnable cleanScanChart = new Runnable() {
        @Override
        public void run() {
            m_vFragmentDataMeasure.scanView = new ScanningService(m_vFragmentDataMeasure.getContext(), false);
            m_vFragmentDataMeasure.scanChart.removeAllViews();
            m_vFragmentDataMeasure.scanChart.addView(m_vFragmentDataMeasure.scanView);
        }
    };
        ...
        generatReport.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (ContextCompat.checkSelfPermission(MainActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                    ...
                    m_vFragmentDataMeasure.mService.resetXYAxis();
                    if (Math.abs(m_vFragmentDataMeasure.mService.getMultipleSeriesRenderer().getXAxisMax()-0.1)<0.0001&&
                            Math.abs(m_vFragmentDataMeasure.mService.getMultipleSeriesRenderer().getYAxisMax()-300)<0.0001&&
                            Math.abs( m_vFragmentDataMeasure.mService.getMultipleSeriesRenderer().getYAxisMin())<0.0001){
                ...
                if (ifFirstStart) {
                    ...
                    if (flag) {
                        ...
                        DataProcessThread dataProcessThread = new DataProcessThread(
                                stopHandler,
                                m_vFragmentDataMeasure.mService,
                                m_vSecondFragmentDataMeasure.mService,
                                m_vSecondFragmentDataMeasure.mService1);
