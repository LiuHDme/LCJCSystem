package com.tomatoLCJC.usbutil;
...
public class SerialPortOpe {
    ...
    //现有设备发送最多通道数据
    final static int CURRENT_CHANEEL=9;
...
//            Log.i(TAG,"setVIDPID Error");
    ...
    public int getDeviceCount(){
        if(ftdid2xx!=null){
            ...
            if (devcount==0){
                ...
                Toast.makeText(mContext, "已连接设备数：" + devcount, Toast.LENGTH_SHORT).show();
            }
            return devcount;
        }
        return 0;
    ...
    public void createDeviceList() {
        if(ftdid2xx!=null) {
            ...
            Toast.makeText(mContext, "设备未连接", Toast.LENGTH_LONG).show();//ftd2xx is null
        }
    }
...
//                Log.e(TAG, "SendMessage: device not open");
            ...
            Toast.makeText(mContext, "设备未连接", Toast.LENGTH_LONG).show();   //ftDev is null ,please connect device
        }
    }
            ...
            synchronized (ftDev) {
                if (ftDev.isOpen()) {
                    ...
                    if (byteCount > 0) {
                        ...
                        byte[] readData = new byte[readLength];
                        ftDev.read(readData, byteCount);
//                        for (int i=0;i<byteCount-CURRENT_CHANEEL*2;i++){
//                            if (readData[i]==0x08&&readData[i+1]==0x5A&&readData[i+CURRENT_CHANEEL*2]==0x08&&readData[i+CURRENT_CHANEEL*2+1]==0x5A){
//                                int nChannelNumber=SystemParameter.getInstance().nChannelNumber;
//                                for (int j=0;j<nChannelNumber+2;j++){
//                                    data.add(NumberHelper.bytesToInt(new byte[]{readData[i+j*2],readData[i+1+j*2]},0));    //低位在前转换
////                                    Log.d("values",String.valueOf(data.get(data.size()-1)));
////                                    Log.d("values-1",String.valueOf(NumberHelper.bytesToInt(new byte[]{readData[i+j*2],readData[i+1+j*2]},0)));
//                                }
//                            }
//                        }
                    }
        ...
        if(ftDev!=null) {
            synchronized (ftDev) {
                if (ftDev.isOpen()) {
                    int byteCount = ftDev.getQueueStatus();
                    if (byteCount > 0) {
                        if (byteCount > readLength){       //byteCount==16320   readLength==32760
                            byteCount = readLength;
                        }
                        readData = new byte[byteCount];
                        ftDev.read(readData, byteCount);
                    }
                }
            }
    ...
    public void enableRead() {
        if(ftDev!=null) {
            ...
            Toast.makeText(mContext, "设备未连接", Toast.LENGTH_SHORT).show();   //ftDev is null!
        }
    }
