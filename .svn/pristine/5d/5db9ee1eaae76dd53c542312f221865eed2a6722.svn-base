...
public class ScanningService extends View implements View.OnTouchListener {
    ...
    public float xScale = 1, yScale = 1;           // 分别控制 x 和 y 方向的缩放程度
    ...
    public float xDistance;        // x 方向总距离
    ...
    private void drawCoordinates(Canvas canvas) {
        ...
        canvas.translate((float) xTranslate * xScale , 0);        // 使坐标跟着图形一起平移
        int n = 1;
        if (xDistance < 1)
            n = 1;
        ...
        canvas.translate(0, (float) yTranslate * yScale);        // 使坐标跟着图形一起平移
        for (int i = 0; i < 20; i++) {
            canvas.drawText(
                    String.valueOf(i),
                    -5f,
    ...
    private void drawScanningChart(Canvas canvas) {
        ...
        float px = (float) (canvasWidth / 2 - xTranslate);
        float py = (float) (canvasHeight / 2 - yTranslate);
                ...
                if (event.getPointerCount() == 1 && event.getAction() != 261
                    ...
                    if (this.getResources().getConfiguration().orientation ==
                        ...
                        FragmentDataMeasure.getInstance().mService.getMultipleSeriesRenderer()
                                .setXAxisMin(-xTrans - (xDistance / xScale - xDistance) / 2);
                        FragmentDataMeasure.getInstance().mService.getMultipleSeriesRenderer()
                                .setXAxisMax(xDistance - xTrans + (xDistance / xScale - xDistance) / 2);
                        // 重绘折线图
